...
def test_prediction_pipeline_ignores_every_point_without_labels() -> None:
    sample = make_sample(points=_lag_cloud([0.0, 0.1], num_current=1))

    output = PreparePointSegInput(ignore_index=-1, require_labels=False)(sample)

    assert np.array_equal(output.segment.labels, np.array([-1, -1], dtype=np.int64))


def test_training_pipeline_requires_loaded_labels() -> None:
    sample = make_sample(points=_lag_cloud([0.0, 0.1], num_current=1))

    with pytest.raises(ValueError, match="requires loaded segmentation labels"):
