...
"""Tests for the camera image resizing transforms."""
...
def test_crop_and_scale_preserves_shape_and_updates_intrinsics() -> None:
    ...
    output = CropAndScale(p=1.0, crop_ratio=0.8)(sample)

    assert output.calibration.image.shape == sample.calibration.image.shape
    assert output.calibration.image.dtype == np.float32
    assert not np.allclose(
        output.calibration.data.new_camera_matrix, sample.calibration.data.new_camera_matrix
    )


def test_crop_and_scale_does_not_mutate_the_input() -> None:
    np.random.seed(0)
    sample = _make_sample()

    CropAndScale(p=1.0, crop_ratio=0.8)(sample)
