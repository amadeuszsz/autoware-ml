...
def keyframe_lidar_frame(sample: Sample) -> LidarFrameDataModel:
    ...
    frame of the sample and the following frames are its sweeps, captured before or after it.
    The keyframe flag marks a frame the dataset annotates, so in a corpus annotated at every
    frame the sweeps carry it as well and the position decides.
