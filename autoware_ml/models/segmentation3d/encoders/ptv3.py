...
def rotary_span(head_dim: int) -> int:
    """Return how many leading head dimensions an axis-split 3D RoPE can rotate.

    The head dimension is split into three equal per-axis chunks, and each
    chunk pairs dimension ``i`` with ``i + chunk // 2``. The chunk width must
    therefore be even, so the rotated span is the largest multiple of six that
    fits. Note that divisibility by three alone is *not* sufficient: an odd
    chunk width mispairs the halves and silently stops being a rotation.
...
class Point3DRoPE(nn.Module):
    ...
    Positions come from integer voxel coordinates rather than metric ones, so
    the encoding is exactly reproducible in the exported graph, where per-stage
    ``grid_coord`` is already part of the input contract while mean-pooled
    metric coordinates are not.

    Any trailing ``head_dim % 6`` dimensions are passed through unrotated. That
    keeps tensor-core-friendly head dimensions usable - at ``head_dim=32`` only
    two of thirty-two channels go unrotated - and partial rotation is standard
    practice in rotary-embedding implementations.
    ...
    def __init__(self, head_dim: int, base: float) -> None:
        ...
        Args:
            ...
            base: Geometric base of the frequency ladder, shared by all three
                ...
                the two have not been matched numerically, so a checkpoint
                trained against that operator needs verification before reuse.

        Raises:
            ValueError: Raised when ``head_dim`` is too small to rotate.
        """
        super().__init__()
        rotated = rotary_span(head_dim)
        if rotated == 0:
...
class PointTransformerV3Encoder(PointModule):
    ...
    def __init__(
        ...
        stem_type: str = "linear",
        enc_conv: Sequence[bool] | None = None,
        enc_attn: Sequence[bool] | None = None,
        ...
        Args:
            ...
            enc_conv: Per-stage flag for the submanifold-convolution positional
                encoding. ``None`` enables it everywhere, which is the PTv3
                default.
            enc_attn: Per-stage flag for attention and its MLP. ``None`` enables
                it everywhere. Stages with attention disabled need no
                serialization order, which the export contract reflects.
    ...
    @property
    def needs_base_serialization(self) -> bool:
        """Return whether stage 0 attends, and therefore needs a level-0 order.

        When the finest stage carries no attention, nothing in the graph reads
        the base serialization, so ``serialized_code`` drops out of the export
        contract along with the input-resolution sorts it feeds.
        """
        return bool(self.enc_attn[0])

    def pooling_stage_needs_order(self, pooling_index: int) -> bool:
        """Return whether one pooling stage must supply a serialization order.

        Pooling stage ``i`` produces the input to encoder stage ``i + 1``, so it
        only needs an order when that stage carries attention.

        Args:
            pooling_index: Index of the pooling stage, ``0`` for the first.
        """
        return bool(self.enc_attn[pooling_index + 1])
...
class LitePTEncoder(PointTransformerV3Encoder):
    ...
    LitePT keeps PTv3's serialization, pooling, and export contract and changes
    only which operations run at which resolution. Early stages are pure
    submanifold convolution and late stages are pure attention with rotary
    position embedding, so the two subsystems never share a block. Because it
    subclasses the PTv3 encoder, every PTv3 task model, head, and export path
