...
(max-probability confidence and normalized entropy, reduced from the softmax at update because
the full ``(N, C)`` distribution would dominate the cache), plus the per-frame detection GT
...
class FramePoints:
    ...
    ``confidence`` and ``entropy`` are the per-point max softmax probability and
    normalized predictive entropy, reduced from the full distribution at update.
    """

    coord: np.ndarray  # (N, 3) float
    pred: np.ndarray  # (N,) int predicted class
    target: np.ndarray  # (N,) int ground-truth class
    confidence: np.ndarray  # (N,) float max softmax probability
...
class Segmentation3DPointCloudMetricSuite(MetricSuite[PointCloudSegState]):
    ...
    def _grouped_state_for(
        ...
        frames = [
            self._frame(
                ...
                self.pred[index],
                self.target[index],
                self.grouped_confidence[index],
                self.grouped_entropy[index],
                self.gt_boxes[index],
                self.gt_box_labels[index],
                metric_range,
                None if column is None else self.region_masks[index][:, column].cpu().numpy(),
    ...
    def _frame(
        ...
        # Boxes carry no region membership of their own, clip only by range.
        if metric_range is not None and boxes_np.shape[0]:
            box_keep = self._range_mask(boxes_np[:, :2], metric_range)
