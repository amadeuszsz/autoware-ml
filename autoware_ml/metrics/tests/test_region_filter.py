    ...
    boxes = np.array(
        [
            [
                ...
                0.0,
                0.0,
            ],  # footprint y in [2, 6] -> overlaps road
...
def test_region_filter_margin_erodes_outer_border_only() -> None:
    ...
    assert eroded.name == "region_road_m0.5"


def test_region_filter_expand_grows_into_off_map_space_only() -> None:
    ...
    expanded = RegionFilter(["road"], _StubProvider(lanelet_map), margin=0.5, expand=True)
    ...
    assert expanded.name == "region_road_x0.5"
...
def test_lanelet_map_speed_at_lookup(tmp_path) -> None:
    ...
    assert lanelet_map.speed_at(50.0, 50.0, default=99.0) == pytest.approx(
        99.0
    )  # outside -> default
