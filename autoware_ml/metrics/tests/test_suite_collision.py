...
def test_suite_computes_ttc_and_criticality_metrics() -> None:
    ...
    # GT: a same-speed lead car (unreachable) + a barrier ahead (reachable).
    ...
    assert gt_ttc[0] == inf  # same-speed lead car
    ...
    assert b1["critical_fn_0p5m"] == 0.0  # both GT either matched or unreachable
