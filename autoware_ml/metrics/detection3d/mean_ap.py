...
class MeanAP(Metric[DetectionState]):
    ...
    def evaluate(self, state: DetectionState, stage: EvalStage) -> dict[str, float]:
        ...
        """
        # One class-set rule for both stages, so the model-selection number and
        # the reported test number average the same classes, and the monitored
        # key is always present (NaN when nothing is measurable, never absent).
