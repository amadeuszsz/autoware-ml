...
class Detection3DMetricSuite(MetricSuite[DetectionState]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Optional collision provider (CollisionTTC). When set, the
        # per-box time-to-collision to ego is computed once per frame at update and
        # carried on each sample for the criticality metrics (B1 critical set, B2
        # weight). Requires ego2global + scene_token in the eval output.
    ...
    def state_for(
        ...
        Args:
            ...
            metric_filter: Optional region filter, boxes are pre-selected by the
                keep-mask computed for it at update time.
