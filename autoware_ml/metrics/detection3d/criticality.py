...
:mod:`autoware_ml.metrics.geometry.reachability`, and the criticality metrics (B1,
B2) read a per-box TTC the suite has already computed. This module holds only the
...
def greedy_match_thresholds(
    ...
    claim pass. Returns ``{threshold: (is_tp, matched_gt)}`` with the arrays
    aligned to the predictions in their original order (``matched_gt`` is ``-1``
    for non-matches).
    """
...
def greedy_match(
    ...
    Returns ``(is_tp, matched_gt)`` aligned to the predictions: ``is_tp[p]`` and
    the matched GT index (``-1`` if none), in the original prediction order.
    """
