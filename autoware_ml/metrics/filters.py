...
from autoware_ml.metrics.base import MetricFilter
...
def ego_collision_agent(
    lanelet_map, ego2global: Any, max_speed_mps: float, body_radius: float
) -> Agent:
...
class RegionFilter(MetricFilter):
    ...
    adjusted by a margin that either cuts inward or grows outward.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            margin: Border adjustment in meters, must be non-negative.
            expand: Direction of the margin. False (default) cuts inward, so
                outer-border points stop counting while internal borders between
                adjacent regions stay intact. True grows outward, so the region
        ...
        if margin < 0.0:
            raise ValueError(
                "margin is a distance and must be >= 0, use expand to pick the direction."
            )
...
class CollisionFilter(MetricFilter):
    ...
    the criticality metrics (B1, B2) use, so any other metric can be reported
    in-path too.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            ego_body_m: Assumed ego half width in meters.
            name: Display name prefixing the metric keys.
        """
        if not region:
    ...
    def _ego_region(self, context: dict[str, Any]):
        ...
        ego = ego_collision_agent(
            lanelet_map, context["ego2global"], self.max_speed_mps, self.ego_body_m
        )
        region = reachable_region(ego, self.params, drivable)
