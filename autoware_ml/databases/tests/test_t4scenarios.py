...
def test_entries_carry_the_dataset_parameters(tmp_path) -> None:
    ...
    assert entry.max_past_sweeps == 2
    assert entry.max_future_sweeps == 1
...
def test_dataset_parameters_are_validated_at_construction() -> None:
    ...
    with pytest.raises(ValidationError, match="max_past_sweeps"):
        _params(max_past_sweeps=-1)
    with pytest.raises(ValidationError, match="max_future_sweeps"):
        _params(max_future_sweeps=-1)
