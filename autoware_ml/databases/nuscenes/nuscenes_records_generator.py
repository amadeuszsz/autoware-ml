...
class NuscenesRecordsGenerator:
    ...
    def _extract_sample_record(
        ...
        past_sweep_data_models = self._extract_lidar_sweeps(
            lidar_frame_data_model, scenario, SweepDirection.PAST
        )
        future_sweep_data_models = self._extract_lidar_sweeps(
            lidar_frame_data_model, scenario, SweepDirection.FUTURE
        )
        ...
        return DatasetRecord(
            ...
            lidar_frames=[
                ...
                *past_sweep_data_models,
                *future_sweep_data_models,
            ],
    ...
    def _extract_lidar_sweeps(
        ...
        """
        Extract the lidar frames on one side of a nuScenes sample, nearest first, at most the
        number the dataset declares for that direction.
        ...
        Returns:
          Sequence[LidarFrameDataModel]: Lidar sweep frame data models ordered from the
        ...
        for _ in range(scenario.max_sweeps(direction)):
            # Stop processing when the chain ends on this side of the sample
            if not current_sample_data_record[direction.link]:
                break

            current_sample_data_record = self.nuscenes_dataset.get(
                "sample_data", current_sample_data_record[direction.link]
            )
            ...
            lidar_frame_data_models.append(
                LidarFrameDataModel(
                    ...
                    lidar_pointcloud_num_features=scenario.lidar_pointcloud_num_features,
