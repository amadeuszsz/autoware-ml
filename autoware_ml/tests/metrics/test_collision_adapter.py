...
def test_adapter_class_dispatch_and_key_cases() -> None:
    adapter = _adapter()
    boxes = np.array([
        _box(25.0, 0.0),   # car ahead, same speed: lead -> inf
    ...
    assert ttc.shape == (4,)
    assert ttc[0] == inf                 # same-speed lead
...
def test_adapter_rejects_vru_class_without_run_speed() -> None:
    road = box(-10.0, -10.0, 10.0, 10.0)
    try:
        CollisionTTC(
            ("car", "wheelchair"),
        ...
        raise AssertionError("expected ValueError for a VRU class without a run speed")
